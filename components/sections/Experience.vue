<template>
  <section id="experience" class="mb-16 scroll-mt-16 md:mb-24 lg:mb-36 lg:scroll-mt-24" aria-label="Work experience">
        ...
        <div>
                    ...
                    <div class="group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
                      ...
                      <div class="z-10 sm:col-span-6">
                        <h3 class="font-medium leading-snug text-slate-200">
                          <div>
                            <a class="inline-flex items-baseline font-medium leading-tight text-slate-200 hover:text-teal-300 focus-visible:text-teal-300  group/link text-base" href="" target="_blank" rel="noreferrer" aria-label="Lead Engineer at Upstatement">
            ...
            <div class="mt-12">
              <a class="inline-flex items-center font-medium leading-tight text-slate-200 font-semibold text-slate-200 group" aria-label="View Full Résumé" href="">
                <span>
...
export default {
  ...
  async fetch() {
    this.jobs = await this.$content('jobs')
      .only(['company', 'description', 'years', 'languages'])
      .sortBy('order', 'desc')
      .fetch()
  },
  computed: {
